...
BASE_PATH=/home/centos/admin-application
...
# 쉬고 있는 set 찾기: admin-dev1이 사용중이면 admin-dev2가 쉬고 있고, 반대면 admin-dev1이 쉬고 있음
if [ $CURRENT_PROFILE == admin-dev1 ]
then
  IDLE_PROFILE=admin-dev2
  IDLE_PROFILE_KILL=admin-dev1
  IDLE_PORT=18081
elif [ $CURRENT_PROFILE == admin-dev2 ]
then
  IDLE_PROFILE=admin-dev1
  IDLE_PROFILE_KILL=admin-dev2
  IDLE_PORT=18082
else
  echo "> 일치하는 Profile이 없습니다. Profile: $CURRENT_PROFILE"
  echo "> admin-dev1을 할당합니다. IDLE_PROFILE: admin-dev1"
  IDLE_PROFILE=admin-dev1
  IDLE_PROFILE_KILL=admin-dev2
  IDLE_PORT=18082
fi
...
echo "> Port 전환"
echo "set \$service_url http://127.0.0.1:${IDLE_PORT};" |sudo tee /etc/nginx/conf.d/service-admin-url.inc
