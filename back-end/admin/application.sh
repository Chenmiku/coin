...
# 쉬고 있는 set 찾기: admin-prod1이 사용중이면 admin-prod2가 쉬고 있고, 반대면 admin-prod1이 쉬고 있음
if [ $CURRENT_PROFILE == admin-prod1 ]
then
  IDLE_PROFILE=admin-prod2
  IDLE_PROFILE_KILL=admin-prod1
  IDLE_PORT=18081
elif [ $CURRENT_PROFILE == admin-prod2 ]
then
  IDLE_PROFILE=admin-prod1
  IDLE_PROFILE_KILL=admin-prod2
  IDLE_PORT=18082
else
  echo "> 일치하는 Profile이 없습니다. Profile: $CURRENT_PROFILE"
  echo "> admin-prod1을 할당합니다. IDLE_PROFILE: admin-prod1"
  IDLE_PROFILE=admin-prod1
  IDLE_PROFILE_KILL=admin-prod2
  IDLE_PORT=18082
fi
